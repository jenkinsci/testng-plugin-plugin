...
public class MethodResult extends BaseResult {
    ...
    /**
     * unique id for this test's run (helps associate the test method with related configuration
     * methods)
     */
    ...
    */
    ...
    private hudson.util.Graph getGraph(final StaplerRequest2 req, StaplerResponse2 rsp) {
        ...
        return new Graph(-1, 800, 150) {
            protected JFreeChart createGraph() {
                return GraphHelper.createMethodChart(
                        req,
                        dataSetBuilder.build(),
                        statusMap,
                        // getUrl instead of getUpUrl as latter gets the complete url and we only
                        // need
    ...
    /**
     * Populates the data set build with results from any successive and at max 9 previous builds.
     *
     * @param dataSetBuilder the data set
     * @param statusMap key as build and value as the execution status (result) of test method
     *     execution
     */
    ...
    /**
     * Gets the method result, if any, from the given set of test results. Searches for method
     * result that matches the url of this method
     *
    ...
    private MethodResult getMethodResult(TestNGResult results) {
        ...
        if (packageMap.containsKey(methodPackageName)
            ...
            List<ClassResult> classResults = packageMap.get(methodPackageName).getChildren();
